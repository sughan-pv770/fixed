...
def answer_query(user_id: int, query: str) -> str:
    ...
    prompt = f"""You are the document-scoped assistant for DocKey AI.
    
You must answer the user's question using ONLY the retrieved content below.
If the answer is not found in the context, you must respond with exactly:
"I couldn't find that information in your uploaded documents."
Do not use any outside knowledge. Do not fabricate or invent information.
Maintain a clean, professional, simple, and user-friendly tone.
If available, cite the source document name.
...
"""
    ...
    try:
        with urllib.request.urlopen(req) as f:
            res = json.loads(f.read().decode("utf-8"))
            return res.get("response", "Error: No response generated.")
    except urllib.error.HTTPError as e:
        return f"System Error: {e.code} API failure."
    except Exception as e:
